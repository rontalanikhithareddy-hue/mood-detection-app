...
with col_left:
    ...
    if st.session_state.last_frame_rgb is not None:
        ...
        st.warning("👁️ Provide an image from the browser camera or upload a file to detect emotion.")
...
with col_middle:
    ...
    if st.session_state.last_emotion is not None and st.session_state.last_confidence is not None:
        ...
        st.warning("👁️ No face detected yet. Enable live stream to start detection.")
    st.markdown('</div>', unsafe_allow_html=True)
